...
@section('title')
Verificaciones - @lang('translation.business-center')
@endsection
...
@section('content')
    ...
    @endphp
    <x-page-header :title="$verificationsTitle" icon="check-square" :has_team="false" />
    <br>
    <div class="container-fluid">
        <div class="row mb-4">
            <div class="col-12 d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="section-title">Lista de Verificaciones</h5>
                    <p class="text-muted">Gestión de requerimientos de verificación por tipo de agua</p>
                </div>
                <a href="{{ route('verifications.create') }}" class="btn-primary">
                            ...
                            <tr class="text-center">
                                <td>
                                    <div class="action-buttons d-flex justify-content-center gap-2">
                                        <a href="{{ route('verifications.edit', $verification) }}" class="btn-warning" title="Editar">
                                            <i data-feather="edit"></i>
                                        </a>
                                        <button class="btn-success" onclick="updateWaterTypes({{ $verification->id }})" title="Gestionar Tipos de Agua">
                                            <i data-feather="droplet"></i>
                                        </button>
                                        <button class="btn-danger" onclick="deleteVerification({{ $verification->id }})" title="Eliminar">
                                            <i data-feather="trash-2"></i>
                                        </button>
                                    </div>
                                </td>
                                <td>
                                    <div class="fw-bold">{{ $verification->name }}</div>
                                </td>
                                <td>
                                    <div>{{ $verification->description ?: 'Sin descripción' }}</div>
                                </td>
                                <td>
                                    <div class="d-flex flex-wrap justify-content-center gap-1">
                                        @foreach($verification->waterTypes as $waterType)
                                ...
                                <td>
                                    <div>{{ $verification->creator->name ?? 'N/A' }}</div>
                                </td>
                                <td>
                                    <div>{{ $verification->created_at->format('d/m/Y H:i') }}</div>
                                </td>
                            </tr>
                        @endforeach
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="waterTypesForm">
                    @csrf
                    <input type="hidden" id="verificationId" name="verification_id">
                    <div class="mb-3">
...
@push('scripts')
<script>
function updateWaterTypes(verificationId) {
    // Limpiar formulario
    document.getElementById('waterTypesForm').reset();
    document.getElementById('verificationId').value = verificationId;
    
    // Obtener los tipos de agua actuales de la verificación
    fetch(`/verifications/${verificationId}/water-types`)
...
function deleteVerification(verificationId) {
    if (confirm('¿Está seguro de que desea eliminar esta verificación?')) {
        fetch(`/verifications/${verificationId}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            }
